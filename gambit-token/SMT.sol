...
contract SMT is IERC20, ISMT, ITimelockTarget {
    ...
    // by disabling the selling of SMT
    ...
    // - only allowing removal of SMT liquidity and no other actions
    // during the migration phase
    mapping(address => bool) public allowedMsgSenders;

    modifier onlyGov() {
        require(msg.sender == gov, "SMT: forbidden");
        _;
    }

    modifier onlyAdmin() {
        require(admins[msg.sender], "SMT: forbidden");
        _;
    }
    ...
    function setNextMigrationTime(uint256 _migrationTime) external onlyGov {
        require(_migrationTime > migrationTime, "SMT: invalid _migrationTime");
    ...
    function beginMigration() external override onlyAdmin {
        require(
            ...
            "SMT: migrationTime not yet passed"
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "SMT: transfer from the zero address");
        require(_recipient != address(0), "SMT: transfer to the zero address");

        if (hasActiveMigration) {
            require(allowedMsgSenders[msg.sender], "SMT: forbidden msg.sender");
            require(!blockedRecipients[_recipient], "SMT: forbidden recipient");
        }

        balances[_sender] = balances[_sender].sub(
            _amount,
            "SMT: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) private {
        require(_account != address(0), "SMT: mint to the zero address");
    ...
    function _approve(
        ...
        require(_owner != address(0), "SMT: approve from the zero address");
        require(_spender != address(0), "SMT: approve to the zero address");
