...
import "./interfaces/ISrxIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/ISrxMigrator.sol";

contract SrxMigrator is ReentrancyGuard, ISrxMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "SrxMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "SrxMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "SrxMigrator: already initialized");
        require(
            _whitelistedTokens.length == _iouTokens.length,
            "SrxMigrator: invalid _iouTokens.length"
        );
        require(
            _whitelistedTokens.length == _prices.length,
            "SrxMigrator: invalid _prices.length"
        );
        require(
            _whitelistedTokens.length == _caps.length,
            "SrxMigrator: invalid _caps.length"
        );
        require(
            _lpTokens.length == _lpTokenAs.length,
            "SrxMigrator: invalid _lpTokenAs.length"
        );
        require(
            _lpTokens.length == _lpTokenBs.length,
            "SrxMigrator: invalid _lpTokenBs.length"
        );
    ...
    function migrate(address _token, uint256 _tokenAmount) public nonReentrant {
        require(
            isMigrationActive,
            "SrxMigrator: migration is no longer active"
        );
        require(
            whitelistedTokens[_token],
            "SrxMigrator: token not whitelisted"
        );
        require(_tokenAmount > 0, "SrxMigrator: invalid tokenAmount");
        ...
        if (hasMaxMigrationLimit) {
            ...
            require(
                ...
                "SrxMigrator: maxMigrationAmount exceeded"
            );
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(srxPrice);
        require(mintAmount > 0, "SrxMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(
            tokenAmounts[_token] < caps[_token],
            "SrxMigrator: token cap exceeded"
        );
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "SrxMigrator: invalid tokenA");
            require(tokenB != address(0), "SrxMigrator: invalid tokenB");
        ...
        ISrxIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "SrxMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "SrxMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "SrxMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("SrxMigrator: action not authorized");
        }
        require(
            count >= minAuthorizations,
            "SrxMigrator: insufficient authorization"
        );
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "SrxMigrator: invalid _action");
