...
contract SrxFloor is ReentrancyGuard, TokenManager {
    ...
    modifier onlyHandler() {
        require(isHandler[msg.sender], "SrxFloor: forbidden");
        _;
    }
    ...
    function setBackedSupply(uint256 _backedSupply) public onlyAdmin {
        require(
            ...
            "SrxFloor: invalid _backedSupply"
        );
        backedSupply = _backedSupply;
    }
    ...
    function setMintMultiplier(uint256 _mintMultiplier) public onlyAdmin {
        require(
            ...
            "SrxFloor: invalid _mintMultiplier"
        );
        mintMultiplier = _mintMultiplier;
    }

    // mint refers to increasing the circulating supply
    // the SRX tokens to be transferred out must be pre-transferred into this contract
    function mint(
        uint256 _amount,
        uint256 _maxCost,
        address _receiver
    ) public onlyHandler nonReentrant returns (uint256) {
        require(_amount > 0, "SrxFloor: invalid _amount");
        ...
        require(cost <= _maxCost, "SrxFloor: _maxCost exceeded");
        ...
        IERC20(srx).transfer(_receiver, _amount);
    ...
    function burn(
        ...
        require(amountOut >= _minOut, "SrxFloor: insufficient amountOut");

        backedSupply = backedSupply.sub(_amount);

        IMintable(srx).burn(msg.sender, _amount);
