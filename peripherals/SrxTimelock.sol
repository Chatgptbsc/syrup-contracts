...
import "./interfaces/ISrxTimelock.sol";
...
contract SrxTimelock is ISrxTimelock {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "SrxTimelock: forbidden");
        _;
    }
    ...
    modifier onlyAdminOrHandler() {
        require(
            ...
            "SrxTimelock: forbidden"
        );
        _;
    }

    modifier onlyTokenManager() {
        require(msg.sender == tokenManager, "SrxTimelock: forbidden");
        _;
    }

    modifier onlyRewardManager() {
        require(msg.sender == rewardManager, "SrxTimelock: forbidden");
        _;
    }
    ...
    constructor(
        ...
        require(_buffer <= MAX_BUFFER, "SrxTimelock: invalid _buffer");
        require(_longBuffer <= MAX_BUFFER, "SrxTimelock: invalid _longBuffer");
        admin = _admin;
        buffer = _buffer;
    ...
    function setExternalAdmin(
        ...
        require(_target != address(this), "SrxTimelock: invalid _target");
    ...
    function setBuffer(uint256 _buffer) external onlyAdmin {
        require(_buffer <= MAX_BUFFER, "SrxTimelock: invalid _buffer");
        require(_buffer > buffer, "SrxTimelock: buffer cannot be decreased");
        buffer = _buffer;
    }
    ...
    function setMaxLeverage(
        ...
        require(
            ...
            "SrxTimelock: invalid _maxLeverage"
        );
    ...
    function setFundingRate(
        ...
        require(
            ...
            "SrxTimelock: invalid _fundingRateFactor"
        );
        require(
            _stableFundingRateFactor < MAX_FUNDING_RATE_FACTOR,
            "SrxTimelock: invalid _stableFundingRateFactor"
        );
    ...
    function setFees(
        ...
        require(
            ...
            "SrxTimelock: invalid _taxBasisPoints"
        );
        require(
            _stableTaxBasisPoints < MAX_FEE_BASIS_POINTS,
            "SrxTimelock: invalid _stableTaxBasisPoints"
        );
        require(
            _mintBurnFeeBasisPoints < MAX_FEE_BASIS_POINTS,
            "SrxTimelock: invalid _mintBurnFeeBasisPoints"
        );
        require(
            _swapFeeBasisPoints < MAX_FEE_BASIS_POINTS,
            "SrxTimelock: invalid _swapFeeBasisPoints"
        );
        require(
            _stableSwapFeeBasisPoints < MAX_FEE_BASIS_POINTS,
            "SrxTimelock: invalid _stableSwapFeeBasisPoints"
        );
        require(
            _marginFeeBasisPoints < MAX_FEE_BASIS_POINTS,
            "SrxTimelock: invalid _marginFeeBasisPoints"
        );
        require(
            _liquidationFeeUsd < 10 * PRICE_PRECISION,
            "SrxTimelock: invalid _liquidationFeeUsd"
        );
    ...
    function setTokenConfig(
        ...
        require(_minProfitBps <= 500, "SrxTimelock: invalid _minProfitBps");

        IVault vault = IVault(_vault);
        require(
            vault.whitelistedTokens(_token),
            "SrxTimelock: token not yet whitelisted"
        );
    ...
    ) external override onlyAdminOrHandler {
    ...
    function setInPrivateTransferMode(
        ...
        if (excludedTokens[_token]) {
            ...
            require(
                ...
                "SrxTimelock: invalid _inPrivateTransferMode"
            );
        }
    ...
    function _mint(address _token, address _receiver, uint256 _amount) private {
        ...
        require(
            ...
            "SrxTimelock: maxTokenSupply exceeded"
        );
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(
            ...
            "SrxTimelock: action not signalled"
        );
        require(
            pendingActions[_action] < block.timestamp,
            "SrxTimelock: action time not yet passed"
        );
    }

    function _clearAction(bytes32 _action) private {
        require(pendingActions[_action] != 0, "SrxTimelock: invalid _action");
