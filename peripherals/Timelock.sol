...
contract Timelock is ITimelock {
    ...
    function initSlpManager() external onlyAdmin {
        ISlpManager _slpManager = ISlpManager(slpManager);

        IMintable slp = IMintable(_slpManager.slp());
        slp.setMinter(slpManager, true);

        IUSDG usdg = IUSDG(_slpManager.usdg());
        usdg.addVault(slpManager);

        IVault vault = _slpManager.vault();
        vault.setManager(slpManager, true);
    }

    function initRewardRouter() external onlyAdmin {
        IRewardRouterV2 _rewardRouter = IRewardRouterV2(rewardRouter);

        IHandlerTarget(_rewardRouter.feeSlpTracker()).setHandler(
            rewardRouter,
            true
        );
        IHandlerTarget(_rewardRouter.stakedSlpTracker()).setHandler(
            rewardRouter,
            true
        );
        IHandlerTarget(slpManager).setHandler(rewardRouter, true);
    }
    ...
    function updateUsdgSupply(uint256 usdgAmount) external onlyKeeperAndAbove {
        address usdg = ISlpManager(slpManager).usdg();
        uint256 balance = IERC20(usdg).balanceOf(slpManager);

        IUSDG(usdg).addVault(address(this));

        if (usdgAmount > balance) {
            uint256 mintAmount = usdgAmount.sub(balance);
            IUSDG(usdg).mint(slpManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdgAmount);
            IUSDG(usdg).burn(slpManager, burnAmount);
        }
    ...
    function setShortsTrackerAveragePriceWeight(
        ...
        ISlpManager(slpManager).setShortsTrackerAveragePriceWeight(
            _shortsTrackerAveragePriceWeight
        );
    }

    function setSlpCooldownDuration(
