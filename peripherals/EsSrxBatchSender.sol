...
contract EsSrxBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esSrx;

    constructor(address _esSrx) public {
        admin = msg.sender;
        esSrx = _esSrx;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsSrxBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esSrx).transferFrom(msg.sender, _accounts[i], _amounts[i]);
